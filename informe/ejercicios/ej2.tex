...
%Para desambiguar el algoritmo realizado, decidimos definir un modelo formal del siguiente modo:\newline
%Sea $v$ un curso representado por un intervalo entre su fecha de inicio $i_{v}$ y su fecha de fin $f_{v}$. Sea C un conjunto de curso. La solución %se obtiene encontrando un conjunto maximo de elementos cuyos intervalos no se solapen, esto significa: 
%\par{$$\forall v^{1}, v^{2} \in C, v^{1} \neq v^{2}\ | \ (v^{1}_{f} < v^{1}_{i}) \vee (v^{2}_{f} < v^{1}_{i})$$} 
%esta es condición suficiente ya que el valor de incio de un curso es menor al valor de fin del mismo curso para todos los cursos.
...
\item La complejidad del algoritmo \textbf{push$\_$back} sobre la estructura \textbf{vector} es O(1) amortizado. \footnote{http://en.cppreference.com/w/cpp/container/vector/push$\_$back}. 
