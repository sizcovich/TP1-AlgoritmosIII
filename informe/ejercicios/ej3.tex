...
Para los tests aleatorios, tomamos valores de configuración que limitan algunas características de éstos con el fin de evitar casos en donde el algoritmo se resuelva rápidamente o que dada la cantidad de paredes dentro de la matriz el problema sea reducido a una instancia de menor tamaño. Para evitar esto decidimos que la cantidad de paredes iba a ser del 20\% del total de la cantidad de casilleros y que la cantidad de casilleros importantes iba a ser del 10\%, elegimos estos valores para mantener el tamaño de los casilleros libres lo suficientemente grande como para que no se iguale al tamaño de una instancia menor. Para generar estos casos lo que hicimos fue crear un vector de $n*m$ posiciones donde cada posicion tiene un $1$ (piso) por defecto y luego tomamos el 20\% de ese vector y lo pusimos en $0$ (pared), luego tomamos otro 10\% que no sea pared y lo pusimos en $2$ (importante), luego utilizamos la funcion \textbf{shuffle} para mezclar de forma aleatoria el vector y lo escribimos en la salida respetando el formato de entrada del ejercicio.

Trataremos de mostrar que los tiempos de ejecución de aproximan a la complejidad que calculamos que es $O((n*m)*4^{n*m})$ y que en ciertos casos, a pesar de que las podas bajen los tiempos de ejecución, por las caracteristicas de las instancias igual se puede ver que la funcion crece asintóticamente aproximandose a la cota de complejidad dada.

Las funciones de complejidad fueron aproximadas a los datos de complejidad de cada conjunto de instancias por algoritmos matematicos en donde se modifican constantes multiplicativas y aditivas de forma tal que la función resultante este lo mas cerca posible de los tiempos dados por los resultados, estas constantes no modifican la forma en la que se comporta la funcion de complejidad sino que modifican su posicion y tamaño.

Los tiempos estan dados en nanosegundos, decidimos no hacer reducciones de los valores de entrada por que estos estaban entre los rangos de $1*10^{3}$ y $1*10^{11}$ y nos parece importante mostrar que en algún punto ,en casos que no difieren mucho de su tamaño de instancia, el tiempo de ejecucion se dispara.
...
En este caso generamos instancias que tuvieran todos los casilleros como espacios libres, sin paredes ni lugares importantes, asi maximizamos la cantidad de casilleros que puede intentar usar el algoritmo para probar combinaciones, los tiempos de esta ejecucion son en general bajos debido a que las podas hacen un gran trabajo reduciendo la cantidad de opciones en cada paso, Por ejemplo por cada sensor que ponga la cantidad de casillas que voy a cubrir va a ser de, a lo sumo, todo el ancho y el alto del tablero debido a que no hay paredes que detenga el laser que éste emite, eso reduce notablemente las posibilidades en el siguiente paso.
...
En este caso hicimos lo mismo que antes, solo que forzamos una combinacion de paredes y casilleros importantes que provoca que el algoritmo no encuentre ninguna solución y, de hecho, no haya ninguna solucion de la matriz, de esta manera nos evitamos que entre en juego una de las podas mas improtantes, que es la que guarda el costo de una solución y nunca busca soluciones cuyo costo sea mayor al actual, como en ningún momento encontramos ninguna solución, nunca actualizamos ese costo con lo cual el algoritmo va a seguir intentando todas las posibles combinaciones hasta que concluya que no hay ninguna combinacion posible para resolver el tablero, sin embargo las podas que se producen al colocar un laser siguen haciendo que se resuelve un poco mas rapido, en contraste con el grafico anterior, este tiene un tiempo de ejecución mayor gracias a que logramos evitar que se utilice una poda importante.
...
Este es un test que tiene la particularidad de anular casi todas las podas que pensamos para el ejercicio, la forma de este test tiene el formato de la Figura 23 y es una generalizacion del mismo para tamaños de $n*m$. Por el orden en el cual nuestro algoritmo intenta poner los sensores (primero el mas caros y luego los mas baratos) éste encuentra las soluciones desde la mas cara (poner todos cuatridireccionales) a la mas barata (poner todos bidireccionales) y la poda del costo de la solucion actual no funciona. Poner un sensor no anula (por el laser) ninguna otra casilla por que todas las casillas estan encerradas entre paredes, y las restricciones para colocar importantes o sensores que acabo de colocar no afectan a ninguna otra casilla por que hay paredes de por medio. Su tiempo de ejecucion se extendio tanto que no pudimos llegar a correr los tamaños de test que corrimos para los otros ejemplos.
