...
\subsection{Testing}
Para generar las pruebas de complejidad, generamos diferentes tipos de instancias que nos permitieran poner a prueba diferentes caracteristicas de nuestro algoritmo, como la calidad de las podas y que tanto mejoran los tiempos de la ejecución cuando estas son aplicadas en mas casos, tambien buscamos configuraciones que nos permitieran medir los tiempos de ejecucion tratando de minimizar la efectividad de las podas, por ultimo hicimos un test aleatorio para poder ver el funcionamiento general del algoritmo.

Generar los casos random tiene valores de configuracion que limitan algunas caracteristicas de estas instancias para evitar generar casos en donde el algoritmo de siempre falso o sea posible que dada la cantidad de paredes dentro de la matriz el problema sea reducido a una instancia de menor tamaño, para evitar estos problemas decidimos que la cantidad de paredes iba a ser el 20\% del total de la cantidad de casilleros y que la cantidad de casilleros importantes iba a ser el 10\%, elegimos estos valores para mantener el tamaño de las casillas libres lo suficientemente grande para que no se iguale al tamaño de las casillas de una instancia de menor tamaño y asi poder notar la diferencia a la hora de correr los algoritmos.
